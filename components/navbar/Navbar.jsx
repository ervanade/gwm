...
const Navbar = () => {
  ...
  return (
    <div className={`fixed top-0 left-0 w-full z-50 ${color ? "bg-[#3D3D3D]/80 shadow-md" : "bg-[#3D3D3D]/80"}`}>
      <div className="mx-auto max-w-7xl px-6 lg:px-12 py-4 flex items-center justify-between text-white">
        <Link href={`/${locale}/#hero`}>
          <Image src="/logo-gwm.svg" alt="Logo GWM" width={137} height={60} className="w-[137px] h-[60px]" />
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-6">
          {navItems.map((item, index) => {
            if (item.label.toLowerCase() === "models") {
              return (
                <div key={index} className="relative">
                  <button onClick={() => setMegaMenuOpen(megaMenuOpen === "models" ? null : "models")} className="flex items-center gap-1 hover:text-primary">
                      ...
                      <div className="flex gap-6 mb-6">
                        {modelTabs.map((tab) => (
                          <button
                            ...
                            className={`pb-1 border-b-2 ${activeTab === tab ? "border-sky-500 text-sky-500 font-semibold" : "border-transparent hover:text-sky-500"}`}
                          >
                            {tab.toUpperCase()}
                          </button>
                        ))}
            ...
            if (item.label.toLowerCase() === "discover") {
              return (
                <div key={index} className="relative">
                  <button onClick={() => setMegaMenuOpen(megaMenuOpen === "discover" ? null : "discover")} className="flex items-center gap-1 hover:text-primary">
                    {item.label} <FaChevronDown className="text-xs" />
                  </button>
                  {megaMenuOpen === "discover" && (
                    <div className="absolute top-full mt-2 bg-white text-dark rounded shadow-md p-4 min-w-[200px]">
                      {subMenu.map((sub, i) => (
                        <Link key={i} href={`/${locale}${sub.link}`} className="block px-4 py-2 text-sm hover:bg-gray-100 rounded">
            ...
            return (
              <Link key={index} href={`/${locale}${item.link}`} className="hover:text-primary">
                {item.label}
              </Link>
            );
          })}
